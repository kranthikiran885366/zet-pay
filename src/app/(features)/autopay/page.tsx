...
import { ArrowLeft, Repeat, PlusCircle, Trash2, Loader2, Info, Banknote, CalendarDays, AlertCircle } from 'lucide-react';
...
export default function AutopayPage() {
    ...
    useEffect(() => {
        const fetchMandates = async () => {
            ...
            try {
                ...
                toast({ variant: "destructive", title: "Error", description: "Could not load autopay mandates." });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleCancelMandate = async (mandateId: string) => {
        ...
        try {
            ...
            toast({ variant: "destructive", title: "Cancellation Failed", description: "Could not cancel the mandate." });
        } finally {
            ...
            <main className="flex-grow p-4 space-y-6 pb-20">
                ...
                <Card className="shadow-md">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {!isLoading && mandates.map(mandate => (
                             <Card key={mandate.id} className="p-4 border shadow-sm relative overflow-hidden">
                                <div className="flex justify-between items-start mb-2">
                                     <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">Mandate ID: {mandate.id}</p>
                                     </div>
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                                     ...
                                     <div className="font-medium text-right capitalize">{mandate.frequency}</div>
                                            ...
                                            <AlertDialogHeader>
                                                <AlertDialogTitle>Cancel Autopay Mandate?</AlertDialogTitle>
                                                 <AlertDialogDescription>
                                                     Are you sure you want to cancel the autopay mandate for "{mandate.merchantName}" (ID: {mandate.id})? Payments will no longer be automatically debited.
                                                 </AlertDialogDescription>
                                            </AlertDialogHeader>
                                            <AlertDialogFooter>
