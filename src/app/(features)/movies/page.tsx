...
import { ArrowLeft, Clapperboard, CalendarIcon, Clock, MapPin, Filter, ChevronDown, ChevronUp, Armchair, X, Loader2, Plane, UserCircle, Wallet, Settings, Fuel, Gauge, Users } from 'lucide-react';
...
interface Seat {
    ...
    price: number;
    gridColumn?: string; // Added for CSS grid
    gridRow?: string;    // Added for CSS grid
    isWomenOnly?: boolean; // Added for specific seat types
    isGangway?: boolean; // Added for non-seat spaces
}
...
// Function to generate grid column class for seats
const gridColsClass = (maxCols: number) => `grid-cols-${maxCols}`;
...
export default function MovieBookingPage() {
    ...
    const totalFare = selectedSeats.reduce((sum, seat) => sum + seat.price, 0);
     ...
     const handleConfirmBooking = async () => {
         ...
         if (!auth.currentUser) {
             toast({ variant: "destructive", title: "Login Required", description: "Please log in to confirm booking." });
             // Optionally redirect to login
             // router.push('/login?redirect=/movies');
             return;
         }
         setIsBooking(true);
         try {
    ...
    return (
        <div className="min-h-screen bg-secondary flex flex-col">
            ...
            <header className="sticky top-0 z-50 bg-primary text-primary-foreground p-3 flex items-center gap-4 shadow-md">
                <Link href="/entertainment" passHref> {/* Link back to entertainment hub */}
            ...
            <main className="flex-grow p-0 sm:p-4 flex flex-col md:flex-row gap-4 pb-20">
                ...
                {/* Cinema & Showtime List (Right Side or Full Width on Mobile when movie selected) */}
                 ...
                 <Dialog open={showSeatSelection} onOpenChange={(open) => { if (!open) { setShowSeatSelection(false); } }}>
                         ...
                         <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
                              ...
                              <div className="w-full pt-4 pb-8">
                                ...
                                <svg width="100%" height="30" viewBox="0 0 100 10" preserveAspectRatio="none">
                                    <path d="M 0 10 Q 50 -5 100 10" stroke="hsl(var(--border))" strokeWidth="1" fill="none" />
                                </svg>
                               ...
                               <div className="flex flex-col items-center">
                                  {seatRows.map(row => (
                                     <div key={row} className="flex items-center gap-1 mb-1 w-full justify-center">
                                        ...
                                        <div className={`grid ${gridColsClass(seatMaxCols)} gap-1.5 flex-grow justify-center items-center`}>
                             ...
                             <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs">
                                <div className="flex items-center gap-1.5"><div className="w-4 h-4 border border-input bg-background rounded-xs"></div> Available</div>
...
function SeatButton({ seat, isSelected, onSelect }: SeatButtonProps) {
    ...
    return (
        <Button
             ...
             size="icon"
             className={cn(
                "h-7 text-[10px] font-medium leading-none border rounded", // Base style, smaller height/text
                seat.type === 'sleeper' ? "w-12" : "w-7", // Sleeper wider
                !seat.isAvailable && "bg-gray-300 text-gray-500 cursor-not-allowed border-gray-400 hover:bg-gray-300",
                isSelected && "bg-primary text-primary-foreground",
                seat.isWomenOnly && !isSelected && seat.isAvailable && "border-pink-400 bg-pink-100 text-pink-700 hover:bg-pink-200",
                seat.type === 'Premium' && !isSelected && seat.isAvailable && "border-yellow-500 hover:bg-yellow-50 text-yellow-700",
                seat.type === 'Recliner' && !isSelected && seat.isAvailable && "border-blue-500 hover:bg-blue-50 text-blue-700",
                seat.isAvailable && !isSelected && "hover:bg-primary/10" // Hover effect for available
             )}
