...
export async function middleware(request: NextRequest) {
  ...
  // --- TEMPORARY BYPASS FOR TESTING - ALWAYS ALLOWS ACCESS AND SHOWS HOME ---
  console.log(`[Middleware - DEV MODE] Path: ${pathname}. Allowing access.`);
  // If the request is for the root path itself, let it proceed.
  // The splash page will handle the redirection to home.
  if (pathname === '/splash' || pathname === '/') {
      return NextResponse.next();
  }
  // For other paths, if you want to strictly enforce showing home after splash,
  // you could redirect non-splash/non-home paths during this dev mode.
  // However, typically, direct navigation to features might be desired for testing.
  // For now, let all paths proceed after this log.
  return NextResponse.next();
